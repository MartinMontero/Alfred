...
| goose/ACP sidecar (stage-goose-sidecar, `src/lib/goose`, pinned goose) | Tauri 2 shell, Memory/vault layer | `scripts/stage-goose-sidecar.mjs` pins `EXPECTED_GOOSE_VERSION = '1.48.0'` AND `.github/workflows/release.yml` pins `GOOSE_VERSION: '1.48.0'`; `npx vitest run src/lib/goose` exits 0 |
