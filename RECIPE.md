...
| goose/ACP sidecar (stage-goose-sidecar, `src/lib/goose`, pinned goose) | Tauri 2 shell, Memory/vault layer | `scripts/stage-goose-sidecar.mjs` pins `EXPECTED_GOOSE_VERSION = '1.43.0'` AND `.github/workflows/release.yml` pins `GOOSE_VERSION: '1.43.0'`; `npx vitest run src/lib/goose` exits 0 |
...
| Release lane (`release.yml`, ADR-0009 feed authorship + regression gate) | CI five-job gate, goose/ACP sidecar, Updater | `.github/workflows/release.yml` pins `GOOSE_VERSION: '1.43.0'`, invokes `updater-feed.mjs build` and `updater-feed.mjs verify`, and 100% of its `uses:` lines are pinned to a 40-hex commit SHA |
